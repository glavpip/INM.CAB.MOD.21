...
! heat and SW-rad balance
       ...
       if(ind_change_heat>0) then
        ...
        ! converting heat fluxes to model units (W/m^2)
          ...
          a_swrad=max(a_swrad*confac_heat,0.0)
...
! Air humidity
       ...
       ! converting air specific humidity to model units (kg/kg)    
        a_qatm=max(a_qatm*confac_qatm,0.0)
       endif
...
! SLP
       ...
       if(ind_change_slpr>0) then
          ...
          a_slpr=max(a_slpr*confac_slpr,0.0)
       endif 
...
! Downwelling radiation
       ...
       ! converting downwelling radiation to model units (W/m^2)    
        a_lwr=max(a_lwr*confac_rad,0.0) 
        a_swr=max(a_swr*confac_rad,0.0) 
       endif
...
! Precipitation
       ...
       ! converting rain precipitation to model units (kg/m**2/s)    
        a_rain=max(a_rain*confac_prec,0.0)
         ...
         if(prec_split>0) then 
          call bc_data_read(ss_atmfiles_fname(14),      &
                                      ...
                                      num_rec_prec  )
         ! converting rain precipitation to model units (kg/m**2/s)    
          a_snow=max(a_snow*confac_prec,0.0)
         endif
       ...
       call bc_full_name(ss_ocfiles(2),          & ! original file name
                         ss_ocfiles_fname(2),    & ! full file name
                              ...
                              ind_clim_sss,      & ! index of climaticity (0-climatic data, 1 - real year data)
                                       m_year)     ! current year number
!  forming full file name for liquid runoff       
       call bc_full_name(ss_ocfiles(3),          & ! original file name
                         ss_ocfiles_fname(3),    & ! full file name
                              ...
                              ind_clim_runoff,   & ! index of climaticity (0-climatic data, 1 - real year data)
                                       m_year)     ! current year number
!  forming full file name for solid runoff       
...
!  forming full file name for HLBC       
       call bc_full_name(ss_ocfiles(9),          & ! original file name
                         ss_ocfiles_fname(9),    & ! full file name
                              path2ocssdata,     & ! path to flux data
       ...
       if(ind_change_sss>0) then
           call bc_data_read(ss_ocfiles_fname(2),      &
                                         sss_obs,      &
                                          ...
                                          nnn,nn,      &
                                      num_rec_sss  )
           sss_obs=max(sss_obs*confac_sss, -salref)
       endif
! RUNOFF
     if(ksw_ssbc==3) then
       ...
       if(ind_change_runoff>0) then
           ...
           call bc_data_read(ss_ocfiles_fname(3),      &
                                          ...
                                          nnn,nn,      &
                                      num_rec_runoff  )
           runoff=max(runoff*confac_runoff,0.0) 
          ...
          if(rof_solid>0) then 
           call bc_data_read(ss_ocfiles_fname(4),      &
                                    runoff_solid,      &
                                          ...
                                          nnn,nn,      &
                                      num_rec_runoff  )
           runoff=max(runoff_solid*confac_runoff,0.0)
          endif
           ...
           call bc_data_read(ss_ocfiles_fname(6),      &
                                 ...
                                 numb_of_lqp_max,      &
                                   ...
                                   1,numb_of_lqp,      &
                                             1,nz,     &
                                      num_rec_slbc  )
           slqbw=max(slqbw*confac_slbc,-salref)
       endif
...
! HLBC
       ...
       if(ind_change_hlbc>0) then
           call bc_data_read(ss_ocfiles_fname(9),      &
                                         sshlqbw,      &
                                 numb_of_lqp_max,      &
                                               1,      &
                                   1,numb_of_lqp,      &
                   ...
                   full_filename,    & ! full file name
                       path2flux,    & ! path to flux data
                        ind_clim,    & ! index of climaticity (0-climatic data, 1 - real year data)
                          m_year)      ! current year number
    use iodata_routes
    character*(*), intent(in)::  path2flux,filename
    character*(*), intent(inout):: full_filename
    character fname*64,chyear*4
    integer, intent(in)::ind_clim, m_year
    integer ierr
...
subroutine check_bc_field_change(time_resolution,    &
    ...
    integer, intent(in):: m_month_of_year,           &
                          m_day_of_year,             &
                          m_hour_of_day

    integer, intent(in):: ftype, time_resolution 
    integer, intent(inout)::ind_change 
...
subroutine bc_data_read(filename,      &
    ...
    include 'reclen.fi'
    ! external parameters
    character*(*), intent(in):: filename              !forcing data file
    integer, intent(in):: nx, ny, m1, m2, n1, n2, nrec      !its dimensions
    real(4), intent(inout)::field(nx,ny) !data array   
    integer m,n
    integer ierr
