...
!-------------------------------------------------------------------------------------------------      
 if(ksw_ssbc<=2) then
   ...
   if(ind_change_stress>0) then
        if (rank==0.and.deb_out_scr>0) then
...
!----------------------------------------------------------------------------------------
 if(ksw_ssbc==3) then
  ...
  if (prec_split==0.and.(ind_change_tatm>0.or.ind_change_prec>0)) then
    if (rank==0.and.deb_out_scr>0) write(*,*) 'Getting snow data from total precipitation'
    !if rain and snow are mixed
!$omp parallel do private(m,n,area)        
    do n=ny_start-1, ny_end+1
      do m=nx_start-1, nx_end+1
        if (lu(m,n)>0.5) then
          ...
          area=(tatm(m,n)+1.0)/2.0
          area=min(max(area,0.0),1.0)
          snow(m,n)=rain(m,n)*(1.0-area)
          rain(m,n)=rain(m,n)-snow(m,n)
